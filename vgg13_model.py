...
class Vgg13Small(CNN):
    ...
    def _build(self, num_classes):
        ...
        Dropout after pooling and FC
          X: tensor containing the training examples
          y: tensor containing the target vectors
          y_out: tensor representing the logits
...
class Vgg13Big(CNN):
    ...
    def _build(self, num_classes):
        ...
        Architecture: 2 + 2 + 3 conv + 3 max pool + 2 FC + 1 FC + out
        Dropout after pooling and FC
          X: tensor for the training examples
          y: tensor for the target vectors
          y_out: output layer
          keep_prob: tensorflow float that we will feed back later
        '''
        ...
        with tf.name_scope('conv1_1') as scope:
            kernel = self._weight_variable(shape=[3, 3, 1, 64])
            conv = self._conv2d(X, kernel)
            biases = self._bias_variable([64])
            out = tf.nn.bias_add(conv, biases)
            conv1_1 = tf.nn.relu(out, name=scope)
        ...
        with tf.name_scope('conv2_1') as scope:
            kernel = self._weight_variable([3, 3, 64, 128])
            conv = self._conv2d(pool1, kernel)
            biases = self._weight_variable([128])
            out = tf.nn.bias_add(conv, biases)
            conv2_1 = tf.nn.relu(out, name=scope)
        ...
        # conv3_2
        with tf.name_scope('conv3_2') as scope:
            kernel = self._weight_variable([3, 3, 256, 256])
            conv = self._conv2d(conv3_1, kernel)
            biases = self._bias_variable([256])
            out = tf.nn.bias_add(conv, biases)
            conv3_2 = tf.nn.relu(out, name=scope)
